...
public class CreateModel : PageModel
{
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        var ok = await _subscriptionService.CreatePlanAsync(
            Input.Name,
            Input.Description,
            Input.Price,
            Input.BillingPeriod,
        ...
        if (!ok)
        {
            ModelState.AddModelError("Input.Name", "A plan with this name already exists.");
            return Page();
        }

        TempData["StatusMessage"] = $"Plan '{Input.Name}' created successfully.";
    ...
    public class PlanInput
    {
        ...
        [Range(0, 9999999, ErrorMessage = "Price must be a positive number.")]
        [Display(Name = "Price (₱)")]
